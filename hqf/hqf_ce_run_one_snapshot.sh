...
if [[ "${md_programs^^}" == *"IPI"* ]]; then
    ...
    sed -i "s|<address>.*cp2k.*|<address> ${workflow_id}.${HQ_STARTDATE}.ce.cp2k.${crosseval_folder//tds.}.r-${snapshot_id} </address>|g" ipi.in.*
    sed -i "s|<address>.*iqi.*|<address> ${workflow_id}.${HQ_STARTDATE}.ce..iqi.${crosseval_folder//tds.}.r-${snapshot_id} </address>|g" ipi.in.*
    ...
    while true; do
        if [ -e "/tmp/ipi_${workflow_id}.${HQ_STARTDATE}.ce.cp2k.${crosseval_folder//tds.}.r-${snapshot_id}" ]; then # -e for any fail, -f is only for regular files
            ...
            for bead_folder in $(ls -v cp2k/); do
                ...
                # Updating the input file (directly here before the simulation due to the timestamp in the socket address)
                sed -i "s|HOST.*cp2k.*|HOST ${workflow_id}.${HQ_STARTDATE}.ce.cp2k.${crosseval_folder//tds.}.r-${snapshot_id}|g" cp2k.in.*
...
if [[ "${md_programs^^}" == *"IQI"* ]]; then
    ...
    # Updating the input file (directly here before the simulation due to the timestamp in the socket address)
    sed -i "s|<address>.*iqi.*|<address> ${workflow_id}.${HQ_STARTDATE}.ce.iqi.${crosseval_folder//tds.}.r-${snapshot_id} </address>|g" iqi.in.*
