...
for i in $(eval echo "{1..${nsim}}"); do 
    ...
    if [[ "${opt_programs}" == "cp2k" ]]; then
        mkdir -p opt.k_${bead_configuration}/cp2k
        cp ../../../input-files/cp2k/${inputfile_cp2k_opt} opt.k_${bead_configuration}/cp2k/cp2k.in.opt
        sed -i "s/k_value/${k_current}/g" opt.k_${bead_configuration}/cp2k/cp2k.in.opt
        sed -i "s/bead_configuration/${bead_configuration}/g" opt.k_${bead_configuration}/cp2k/cp2k.in.opt
        sed -i "s/ABC .*/ABC ${A} ${B} ${C}/g" opt.k_${bead_configuration}/cp2k/cp2k.in.opt
