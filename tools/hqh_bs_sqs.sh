...
if [ "${batchsystem}" == "slurm" ]; then
    sacct -ojobid%15,ncpus,jobname%70,partition,state 2>&1 | grep  "^ \+[0-9]\+" | grep -v "^ \+[0-9]\+\." | grep -vE "COMPL|FAILED|CANCEL" || true       # On Odyssey this is needed to filter out all the associated job steps (internal and regular job steps, because we only want infos about the main jobs)
