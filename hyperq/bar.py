...
class BAR:
    ...
    def compute_bar(self):
        ...
        for C in self.values[self.C_filename]:
            ...
            # Second BAR equation
            # DeltaF= -k_b*T*(C - math.log(n_2 / n_1))
...
class BAR_DeltaU:
    def __init__(self, U1_U2_minus_U1_U1_filename, U2_U1_minus_U2_U2_filename, C_filename, temp,
                 outputFilename):
        # The first potential is always the sampling potential, the second one is the evaluating potential.

        # Reading in the values of the input files into lists
        filenames = [U1_U2_minus_U1_U1_filename, U2_U1_minus_U2_U2_filename, C_filename]
        # value_types = ["U1_U1_values", "U1_U2_values", "U2_U1_values", "U2_U2_values", "C_values"]
        self.values = dict.fromkeys(filenames)
        for filename in filenames:
            self.values[filename] = np.loadtxt(filename)
    ...
    def compute_bar(self):
        ...
        for C in self.values[self.C_filename]:
            ...
            sum_1 = 0
            sum_2 = 0
            n_1 = len(self.values[self.U1_U2_minus_U1_U1_filename])
            n_2 = len(self.values[self.U2_U1_minus_U2_U2_filename])  # The number of snapshots from the sampling wrt to U2 -> coordinates of U2 MD
            ...
            # self.delta_F_1_reduced.append(math.log(sum_2 / sum_1) + C - math.log(n_2 / n_1))
            self.delta_F_1.append((self.k_b * self.temp) * (
            math.log(sum_2 / sum_1) + C - math.log(n_2 / n_1)))  # https://en.wikipedia.org/wiki/Boltzmann_constant
            self.C_values_accepted.append(C)
